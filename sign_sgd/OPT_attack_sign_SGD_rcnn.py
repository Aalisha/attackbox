...
from obj_detection import obj_prediction
...
class OPT_attack_sign_SGD_rcnn(object):
    def __init__(self, model, k=200, train_dataset=None):
        self.model = model
        self.k = k
        self.train_dataset = train_dataset

    def process_y(self, y0):
        boxes = []
        classes = []
        #print(y0)
        for y in y0:
                #print(y)
                boxes.append(np.array(y['bbox']))
                classes.append(np.array(y['category_id']))

        return boxes, classes

    def model_prediction(self, x0, y0):
        print('Model prediction')
        model = self.model
        #if one of the model makes false prediction
        threshold = 0.6
        boxes, classes = self.process_y(y0)
        pred_boxes, pred_class, pred_labels = obj_prediction(x0,model, threshold)
        print('Object Prediction')
        print(pred_boxes, pred_class, pred_labels)
        print('Actual Prediction')
        print(boxes, classes)
        #Can we assume that the segments have one-to-one mapping
        #First we will have to write the code for actual y0 for the model which is obtained after processing y0
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.001, iterations = 1000, query_limit=20000,
        ...
        """
        model = self.model
        if (not self.model_prediction(x0, y0)):
